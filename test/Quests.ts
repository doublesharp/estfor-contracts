...
describe("Quests", function () {
  ...
  describe("Strategies", function () {
    describe("Brush buying strategy", function () {
      ...
      it("Quest completed", async function () {
        ...
        const questId = quest?.questId;
        await players.connect(alice).activateQuest(playerId, questId);
        const balanceBefore = brush.balanceOf(alice.address);
        await quests.connect(alice).buyBrushQuest(alice.address, playerId, 0, {value: 10});
        const balanceAfter = brush.balanceOf(alice.address);
        expect((await balanceBefore).add(100)).to.eq(balanceAfter);
    ...
    it.only("Strategy food giving quest", async function () {
      ...
      const quest = {...quest1, actionChoiceId: ACTIONCHOICE_COOKING_MINNUS, actionChoiceNum: 5};
      await quests.addQuest(quest, false, defaultMinRequirements);
      const questId = quest?.questId;
      await players.connect(alice).activateQuest(playerId, questId);
